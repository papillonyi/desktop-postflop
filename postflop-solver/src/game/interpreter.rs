...
#[inline]
fn decode_signed_slice_into(dst: &mut [f32], slice: &[i16], scale: f32) {
    let decoder = scale / i16::MAX as f32;
    dst.iter_mut().zip(slice).for_each(|(d, &x)| {
        *d = x as f32 * decoder;
    });
}
        ...
        // player node
            ...
            if node.num_actions() > 1 {
                let mut weights = if self.is_compression_enabled {
                    normalized_strategy_compressed_row(
                        node.strategy_compressed(),
                        node.num_actions(),
                        action,
                    )
                } else {
                    normalized_strategy_row(node.strategy(), node.num_actions(), action)
                };

                let locking = self.locking_strategy(&node);
                if !locking.is_empty() {
                    apply_locking_strategy(&mut weights, row(locking, action, num_hands));
                }

                self.apply_swap(&mut weights, player, false);
            ...
            if self.is_compression_enabled {
                let (ptr, scale) = {
                    ...
                    (slice.as_ptr(), node.cfvalue_scale())
                };
                let slice = unsafe { std::slice::from_raw_parts(ptr, num_hands) };
                decode_signed_slice_into(&mut self.cfvalues_cache[player], slice, scale);
            } else {
                let ptr = {
                    let node = self.node();
                    row(node.cfvalues(), action, num_hands).as_ptr()
                };
                let slice = unsafe { std::slice::from_raw_parts(ptr, num_hands) };
                self.cfvalues_cache[player].copy_from_slice(slice);
            }
