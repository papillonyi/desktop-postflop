...
impl PostFlopGame {
    ...
    fn set_bunching_effect_internal(&mut self, bunching_data: &BunchingData) -> Result<(), String> {
        ...
        if self.card_config.river == NOT_DEALT {
            for player in 0..2 {
                ...
                let buf = into_par_iter(0..52)
                    .map(|turn| {
                        ...
                        for &(c1, c2) in player_cards {
                            ...
                            values.reserve(opponent_cards.len());
                            for &(c3, c4) in opponent_cards {
                                let opponent_mask: u64 = (1 << c3) | (1 << c4);
                                if (player_mask | bit_turn) & opponent_mask != 0 {
                                    ...
                                    values.push(bunching_data.result_5cards(mask));
                                }
                            }
                        }
                ...
                self.bunching_num_turn[player] =
                    Self::push_flat_chunks_to_arena_f32(&mut arena, buf);
        ...
        for player in 0..2 {
                    ...
                    for &(c1, c2) in player_cards {
                        ...
                        for &(c3, c4) in opponent_cards {
                            ...
                            if (player_mask | board_mask) & opponent_mask != 0 {
                                ...
                                values.push(bunching_data.result_6cards(mask));
                            }
                        }
                    }
            ...
            self.bunching_num_river[player] = Self::push_flat_chunks_to_arena_f32(&mut arena, buf);
            ...
            let buf = into_par_iter(0..52)
                .map(|turn| {
                    ...
                    for river in 0..52 {
                        ...
                        let player_swap = swap_option
                            .map(|swap| Self::swapped_indices(player_len, &swap[player]));
                        let opponent_swap = swap_option
                            .map(|swap| Self::swapped_indices(opponent_len, &swap[player ^ 1]));
                        ...
                        for (i, inner) in outer.iter_mut().enumerate() {
                            ...
                            if let Some(opponent_swap) = &opponent_swap {
                                inner.iter_mut().zip(opponent_swap).for_each(
                                    |(dst, &opponent_index)| {
                                        let num = arena[index + opponent_index] as f64;
                                        let strength = opponent_strength[opponent_index];
                                ...
                                inner
                                    .iter_mut()
                                    .zip(&arena[index..index + opponent_len])
                                    .zip(opponent_strength)
                                    .for_each(|((dst, num), &strength)| {
                                        #[allow(clippy::comparison_chain)]
                                        if strength < threshold {
                                            *dst += *num as f64;
                                        } else if strength > threshold {
                    ...
                    Self::flatten_outer_f64(outer)
                })
                .collect::<Vec<_>>();

            self.bunching_coef_turn[player] = Self::push_flat_chunks_to_arena_f64(&mut arena, buf);
        }
        ...
        for player in 0..2 {
            ...
            for turn in 0..52 {
                ...
                let player_swap =
                    swap_option.map(|swap| Self::swapped_indices(player_len, &swap[player]));
                let opponent_swap =
                    swap_option.map(|swap| Self::swapped_indices(opponent_len, &swap[player ^ 1]));
                ...
                for (i, inner) in outer.iter_mut().enumerate() {
                    ...
                    if let Some(opponent_swap) = &opponent_swap {
                        inner
                            .iter_mut()
                            .zip(opponent_swap)
                            .for_each(|(dst, &opponent_index)| {
                                *dst += arena[index + opponent_index] as f64;
                            });
                    } else {
                        inner
                            .iter_mut()
            ...
            Self::push_flat_chunks_to_arena_f64(&mut arena, vec![Self::flatten_outer_f64(outer)])
                .into_iter()
                .for_each(|v| {
    ...
    /// Converts swapped pairs into an index map that can be reused without copying data rows.
    fn swapped_indices(len: usize, swap_list: &[(u16, u16)]) -> Vec<usize> {
        let mut indices = (0..len).collect::<Vec<_>>();
        apply_swap(&mut indices, swap_list);
        indices
    }

    /// Flattens rows and stores one-based local offsets in the returned index vector.
    fn flatten_outer_f64(outer: Vec<Vec<f64>>) -> (Vec<usize>, Vec<f64>) {
    ...
    /// Pushes flat chunks to an arena and returns arena indices for each row.
    fn push_flat_chunks_to_arena_f32(
        arena: &mut Vec<f32>,
        chunks: Vec<(Vec<usize>, Vec<f32>)>,
    ) -> Vec<Vec<usize>> {
        let mut ret = Vec::with_capacity(chunks.len());

        for (mut indices, values) in chunks {
            let base = arena.len();
    ...
    /// Pushes flat chunks to an arena and returns arena indices for each row.
    fn push_flat_chunks_to_arena_f64(
        arena: &mut Vec<f32>,
        chunks: Vec<(Vec<usize>, Vec<f64>)>,
    ) -> Vec<Vec<usize>> {
        let mut ret = Vec::with_capacity(chunks.len());
        ...
        for (mut indices, values) in chunks {
            ...
            arena.extend(values.into_iter().map(|v| v as f32));
            ret.push(indices);
        }
