...
#[inline]
pub(crate) fn normalized_strategy_row(
    strategy: &[f32],
    num_actions: usize,
    action: usize,
) -> Vec<f32> {
    let row_size = strategy.len() / num_actions;
    let action_offset = action * row_size;
    let default = 1.0 / num_actions as f32;
    let mut normalized = Vec::with_capacity(row_size);

    for hand in 0..row_size {
        let mut denom = 0.0;
        for action in 0..num_actions {
            denom += strategy[action * row_size + hand];
        }

        let value = strategy[action_offset + hand];
        normalized.push(if is_zero(denom) {
            default
        } else {
            value / denom
        });
...
#[inline]
pub(crate) fn normalized_strategy_compressed_row(
    strategy: &[u16],
    num_actions: usize,
    action: usize,
) -> Vec<f32> {
    ...
    for hand in 0..row_size {
        let mut denom = 0.0;
        for action in 0..num_actions {
            denom += strategy[action * row_size + hand] as f32;
        }

        let value = strategy[action_offset + hand] as f32;
        normalized.push(if is_zero(denom) {
            default
        } else {
            value / denom
...
mod tests {
    ...
    #[test]
    fn normalized_strategy_row_matches_full_normalization() {
        let strategy = vec![0.0, 2.0, 1.0, 1.0, 3.0, 0.0];
        let full = normalized_strategy(&strategy, 3);

        assert_eq!(normalized_strategy_row(&strategy, 3, 1), row(&full, 1, 2));
    }

    #[test]
    fn normalized_strategy_compressed_row_matches_full_normalization() {
        let strategy = vec![0, 2, 1, 1, 3, 0];
        let full = normalized_strategy_compressed(&strategy, 3);

        assert_eq!(
            normalized_strategy_compressed_row(&strategy, 3, 2),
            row(&full, 2, 2)
        );
    }
}
